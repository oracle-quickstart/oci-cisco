...
output "firewallA_instance_public_ips" {
  value = [oci_core_instance.asa-vms[0].*.public_ip]
}

output "firewallA_instance_private_ips" {
  value = [oci_core_instance.asa-vms[0].*.private_ip]
}

output "firewallB_instance_public_ips" {
  value = [oci_core_instance.asa-vms[1].*.public_ip]
}

output "firewallB_instance_private_ips" {
  value = [oci_core_instance.asa-vms[1].*.private_ip]
}

output "instance_https_urls" {
  value = formatlist("https://%s", oci_core_instance.asa-vms.*.public_ip)
}
...
output "fmc1_metadata" {
  value = [oci_core_instance.asa-vms[0].*.metadata]
}

output "fmc2_metadata" {
    value = [oci_core_instance.asa-vms[1].*.metadata]
}
...
username: admin
IP_Address: ${oci_core_instance.asa-vms.0.public_ip}
SSH Key
For example:
$ ssh –i id_rsa admin@${oci_core_instance.asa-vms.0.public_ip}
...
1.  In a web browser, 
    - Connect to the Cisco FTD Firewall-1: https://${oci_core_instance.asa-vms.0.public_ip}
    - Connect to the Cisco FTD Firewall-2: https://${oci_core_instance.asa-vms.1.public_ip}
