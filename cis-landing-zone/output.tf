...
output "fmc_instance_https_url" {
  value = formatlist("https://%s", oci_core_instance.firewall-manager.0.public_ip)
}

output "ftd1_metadata" {
  value = [oci_core_instance.firewall-vms[0].*.metadata]
}

output "ftd2_metadata" {
    value = [oci_core_instance.firewall-vms[1].*.metadata]
}
...
username: admin
IP_Address: ${oci_core_instance.firewall-manager.0.public_ip}
SSH Key
For example:
$ ssh –i id_rsa admin@${oci_core_instance.firewall-manager.0.public_ip}
...
1.  In a web browser, 
    - Connect to the Cisco FMC: https://${oci_core_instance.firewall-manager.0.public_ip}
