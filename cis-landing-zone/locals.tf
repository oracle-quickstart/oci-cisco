...
locals {
  ...
  mp_fmc_subscription_enabled  = var.mp_fmc_subscription_enabled ? 1 : 0
  listing_fmc_id               = var.mp_fmc_listing_id
  listing_fmc_resource_id      = var.mp_fmc_listing_resource_id
  ...
  is_flex_fmc_shape            = var.vm_compute_shape == "VM.Standard.E3.Flex" ? [var.vm_flex_shape_ocpus] : []
}
